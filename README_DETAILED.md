...
```
User Input (Swipe Screen)
        ↓
Preference Collection (4 Directions)
        ↓
...
scoreFood(food, weight):
  - Super Like: weight = 3x
  - Like: weight = 2x
  - Not Sure: weight = 1x
  - Dislike: weight = 0x (negative impact)
```
...
#### 5. **UI/UX Pattern**
```
Swipe Screen (Data Input)
    ↓
Results Screen (Data Visualization)
    ├── Key Highlights Carousel (Diet + Cuisines + Categories)
    ├── Taste Highlights (Lifestyle Insights)
    └── Food Categories Carousel (Detailed Preferences)
```
...
```bash
git clone https://github.com/manii5228/calorai-taste-profile.git
...
│   ├── swipe.tsx            # Swipe interaction screen
│   └── results.tsx          # Results & insights screen
├── components/
│   ├── GlassCard.tsx        # Glass morphism card
│   ├── GlassView.tsx        # Alternative glass container
│   ├── ItemCarousel.tsx     # Food items carousel
│   ├── ProgressBar.tsx      # Progress indicator
│   ├── SwipeActions.tsx     # Action buttons
│   ├── SwipeCard.tsx        # Food card display
│   └── SwipeIndicator.tsx   # Swipe direction badges
├── constants/
│   ├── foods.ts             # Foods & cuisines export
│   ├── foods.json           # Food data (50+ items)
│   └── theme.ts             # Design tokens
├── utils/
│   ├── tasteProfiler.ts     # Profile building logic
│   └── insigths.ts          # Analytics & insights
└── package.json
```
